...
from dataset.augmentation_seg import *
...
def list_add_prefix(txt_path, prefix_1):
    '''
    prefix_1: prefix for training dataset
    prefix_2: imgs, masks etc.
    '''
    ...
    if prefix_1 is not None:
        ...
        # extract unique names (assume name is in some part of the file path, e.g. "ProstateX_0110")
        unique_names = set()
        for line in filtered_lines:

            # get file name part, and extract unique names through rules
            filename = line.split('/')[-1]  # get file name
...
class UnisegDataset(data.Dataset):
    ...
    def id2trainId(self, label, part_id):
        # initialize result mapping, all values set to 0 (表示不存在该类别)
        shape = label.shape
        results_map = torch.zeros((27, shape[0], shape[1], shape[2]), dtype=torch.float32, device=label.device)
        
        # map different classes according to part_id
        if part_id == 0:
    ...
    def pad_image(self, img, target_size,mode):
        ...
        """
        # calculate the size to be padded
        if mode =='image':
        ...
        # if the missing value is less than 0, set to 0
        rows_missing = max(rows_missing, 0)
        cols_missing = max(cols_missing, 0)
        dept_missing = max(dept_missing, 0)

        # use torch.nn.functional.pad to pad
        padded_img = torch.nn.functional.pad(
            img, 
            (0, dept_missing, 0, cols_missing, 0, rows_missing),  # pad order: (depth after, depth before, width after, width before, height after, height before)
            mode='constant',
            value=0  # pad value is 0
        )
        return padded_img
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            for i in range(images.shape[0]):
                images[i] = self.truncate(images[i])#normalize
           
             
            if np.random.rand(1) <= 0.5:
                #  1. generate uniform rotation parameters
        ...
        if name[:3] in ['Bra']:
            ...
            x1 = images[0, ...]  # (t1w)(128,128,96)
            x2, x3, x4, x5, x6,x7,x8 = [torch.zeros_like(images[0, ...]) for _ in range(7)]
            sequence_code = torch.zeros(8, dtype=torch.int32)
            sequence_code[0] = 1
        ...
        # add region code and task code before returning
        # region code: determined by part_id (0-9), converted to one-hot form
        region_ids = torch.zeros(10, dtype=torch.float32)  # 10 regions, initialized to 0
        region_ids[part_id] = 1.0  # set corresponding position to 1
        
        # task code: 0 for segmentation task, 1 for classification task
        # since this is a segmentation dataset, the task code is 0
        task_ids = torch.tensor(0, dtype=torch.long)  # 0 for segmentation task
...
def tr_seg_collate(batch):
    ...
    region_ids = np.stack(region_ids, 0)  # now is [B, 10] one-hot vector
