...
def ProcessImageWithFixedSize(imImage, imLabel, save_path, name, target_spacing=[1.0, 1.0, 1.0],target_size=[128, 128, 128], mode='0000'):
    ...
    if mode=="t1c":
        ...
        # 保存处理后的图像
        sitk.WriteImage(resized_img, '%s/%s/%s_flair.nii.gz'%(save_path, name, name))
        # sitk.WriteImage(resized_lab, '%s/%s/%s_seg.nii.gz'%(save_path, name, name))

if __name__ == '__main__':
    src_path = '/data/zzn/UniMRINet/dataset/seg_orginal/cls/14BraTS/'
    mr_tgt_path = '/data/zzn/UniMRINet/dataset/MR_Dataset_1.5_1.5_1.5/14BraTS/'
    ...
    #     print(name, 'done')
    for name in mr_name_list:
        for mode, suffix in [('t1c', 't1c'), ('t1', 't1n'), ('t2', 't2w'),('t2f', '2f')]:
