...
def ProcessImageWithFixedSize(imImage, save_path, name, target_spacing=[1.0, 1.0, 1.0],target_size=[128, 128, 128], mode='0000'):
    ...
    # 重采样到固定大小
    resized_img = ResampleToFixedSize(imImage, size=target_size, interp=sitk.sitkBSpline)
    # resized_lab = ResampleToFixedSize(cropped_lab, size=target_size, interp=sitk.sitkNearestNeighbor)
    
    if mode=="A":
        # 保存处理后的图像
        sitk.WriteImage(resized_img, '%s/%s/%s_C+A.nii.gz'%(save_path, name, name))
        # sitk.WriteImage(resized_lab, '%s/%s/%s_seg.nii.gz'%(save_path, name, name))
    elif mode=="V":
        # 保存处理后的图像
        sitk.WriteImage(resized_img, '%s/%s/%s_C+V.nii.gz'%(save_path, name, name))
        # sitk.WriteImage(resized_lab, '%s/%s/%s_seg.nii.gz'%(save_path, name, name))

if __name__ == '__main__':
    src_path = '/data/zzn/UniMRINet/dataset/seg_orginal/cls/13LLD/'
    mr_tgt_path = '/data/zzn/UniMRINet/dataset/MR_Dataset_1.5_1.5_1.5/13LLD/'
    ...
    for name in mr_name_list:
        for mode, suffix in [('A', 'C+A'), ('V', 'C+V')]:
            img_path = src_path + f"imagesTr/{name}_{suffix}.nii.gz"
            # lab_path = src_path + f"labelsTr/{name}_{suffix}.nii.gz"
            ...
            ProcessImageWithFixedSize(img, mr_tgt_path, name, target_size=[128, 128, 128], mode=mode)
