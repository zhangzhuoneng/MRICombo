...
# Map string names to convolutional block implementations.
def get_block(name):
    block_map = {
...
# Map normalization keywords to 3D normalization layers.
def get_norm(name):
...
# Per-sequence stem expert used before cross-sequence fusion.
...
# One encoder expert branch inside MoE encoder stages.
...
# One decoder expert branch inside MoE decoder stages.
...
# Gating network that predicts expert probabilities from features
# and optional region/task metadata.
class GatingNetwork(nn.Module):
    ...
    def forward(self, x, region_ids=None, task_ids=None):
        ...
        combined_encoding = torch.cat([region_emb, task_emb], dim=1)

        fused_encoding = self.encoding_fusion(combined_encoding)

        combined_features = torch.cat([pooled_x, fused_encoding], dim=1)
        ...
        return gates
# MRICombo backbone: sequence-expert fusion + hierarchical MoE encoder/decoder
# with optional MAE reconstruction branch for self-supervised learning.
class MRICombo(nn.Module):
    def __init__(self, seg_in_ch, cls_in_ch, base_ch, scale=[2,2,2,2], kernel_size=[3,3,3,3],
        ...
        # Sequence-specific stems for segmentation and classification branches.
        ...
        # MoE experts for four encoder stages.
        ...
        # MoE experts for four decoder stages.
        ...
        # Stage-wise gates condition on features + metadata (region/task).
        ...
        # MAE decoder reconstructs fused feature maps from bottleneck features.
        self.mae_decoder = nn.Sequential(

            nn.ConvTranspose3d(base_ch * 16, base_ch * 8, kernel_size=2, stride=2),
            nn.InstanceNorm3d(base_ch * 8),
            nn.ReLU(inplace=True),

            nn.ConvTranspose3d(base_ch * 8, base_ch * 4, kernel_size=2, stride=2),
            nn.InstanceNorm3d(base_ch * 4),
            nn.ReLU(inplace=True),

            nn.ConvTranspose3d(base_ch * 4, base_ch * 2, kernel_size=2, stride=2),
    ...
    def _apply_input_voxel_masking(self, inputs, mask_ratio=0.5):
        # Patch-wise masking: sample on low-res grid, then upsample to full resolution.
        """Apply voxel-wise masking independently to each input sequence."""
        B = inputs[0].shape[0]
        device = inputs[0].device
        D, H, W = inputs[0].shape[2:]
        ...
        for x in inputs:
            ...
            scale_factor = 16
            d_s, h_s, w_s = D // scale_factor, H // scale_factor, W // scale_factor

            small_mask = (torch.rand(B, 1, d_s, h_s, w_s, device=device) > mask_ratio).float()

            voxel_mask = F.interpolate(small_mask, size=(D, H, W), mode='nearest')

            masked_x = x * voxel_mask

            masked_inputs.append(masked_x)
            input_masks.append(voxel_mask)

        return masked_inputs, input_masks

    # Reset stored gate activations used for load-balance regularization.
    ...
    def get_load_balance_loss(self, coeff=1.0, kind='kl'):
        # Penalize expert collapse by encouraging near-uniform gate usage.
        ...
        return lb_loss
    # Uniformly average available sequences (masked by sequence_code).
    def _compute_equal_weights(self, sequence_code):

        num_sequences = sequence_code.sum(dim=1, keepdim=True)

        num_sequences = torch.clamp(num_sequences, min=1.0)

        equal_weights = sequence_code.float() / num_sequences

        return equal_weights

    # Learn per-sequence fusion weights and renormalize over present inputs.
    ...
    # Top-k routed MoE encoder for one stage.
    ...
    # Top-k routed MoE decoder for one stage.
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8, sequence_code, task, region_ids=None, use_equal_weights=False,
                return_mae_recon=False, mae_mask_ratio=0.5, use_masked_encoder=False):
        """Forward pass for segmentation or classification task."""
        batch_size = sequence_code.shape[0]

        self._reset_lb_buffers()

        if task == "seg":
            # Build task one-hot used by metadata-aware gates.
            task_ids = torch.zeros(batch_size, 2, dtype=torch.float32, device=sequence_code.device)
            task_ids[:, 0] = 1.0
        elif task == "cls":
            # Classification branch reuses MoE encoder and returns bottleneck features.
            task_ids = torch.zeros(batch_size, 2, dtype=torch.float32, device=sequence_code.device)
            task_ids[:, 1] = 1.0
        else:
            print("no task error")
        ...
        if task == "seg":
            ...
            self.mae_mask = None
            self.mae_target = None
            mae_recon = None

            inputs = [x1, x2, x3, x4, x5, x6, x7, x8]

            if return_mae_recon:
                # Use masked inputs for feature extraction and keep masks for MAE loss.

                masked_inputs, input_masks, = self._apply_input_voxel_masking(inputs, mae_mask_ratio)

                self.input_masks = input_masks

                x1_masked, x2_masked, x3_masked, x4_masked, x5_masked, x6_masked, x7_masked, x8_masked = masked_inputs
            else:

                x1_masked, x2_masked, x3_masked, x4_masked, x5_masked, x6_masked, x7_masked, x8_masked = inputs

            x1_features = self.seg_experts[0](x1_masked)
            x2_features = self.seg_experts[1](x2_masked)
            x3_features = self.seg_experts[2](x3_masked)
            x4_features = self.seg_experts[3](x4_masked)
            x5_features = self.seg_experts[4](x5_masked)
            x6_features = self.seg_experts[5](x6_masked)
            x7_features = self.seg_experts[6](x7_masked)
            x8_features = self.seg_experts[7](x8_masked)
            ...
            if return_mae_recon:
                ...
                with torch.no_grad():
                    x1_full = self.seg_experts[0](x1)
                    x2_full = self.seg_experts[1](x2)
                    x3_full = self.seg_experts[2](x3)
                    x4_full = self.seg_experts[3](x4)
                    x5_full = self.seg_experts[4](x5)
                    x6_full = self.seg_experts[5](x6)
                    x7_full = self.seg_experts[6](x7)
                    x8_full = self.seg_experts[7](x8)

                    aggrevate_feature_full = (
                        x1_full * weight[:, 0:1, None, None, None] +
                        x2_full * weight[:, 1:2, None, None, None] +
                        x3_full * weight[:, 2:3, None, None, None] +
                        x4_full * weight[:, 3:4, None, None, None] +
                        x5_full * weight[:, 4:5, None, None, None] +
                        x6_full * weight[:, 5:6, None, None, None] +
                        x7_full * weight[:, 6:7, None, None, None] +
                        x8_full * weight[:, 7:8, None, None, None]
                    )

                    self.mae_target = aggrevate_feature_full.detach()

            skips = []

            x = aggrevate_feature

            # Encoder path with metadata-conditioned MoE routing.
            for i in range(4):
            ...
            self.encoder_bottleneck = x.detach().clone()

            if return_mae_recon:
                # Use masked inputs for feature extraction and keep masks for MAE loss.
                mae_recon = self.mae_decoder(x)

            # Decoder path with skip connections and metadata-conditioned routing.
            for i in range(4):
            ...
            if return_mae_recon:
                ...
                return out, mae_recon, self.mae_target, self.input_masks
    ...
    # Metadata-aware top-k routed encoder (uses region/task ids in the gate).
    ...
    # Metadata-aware top-k routed decoder (uses region/task ids in the gate).
