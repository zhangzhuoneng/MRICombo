...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        # return dec4, logits
        return  logits
    # x1_t1c,x2_t1n,x3_t2f,x4_t2w,x5_dce,x6_adc,x7_dwi
    def forward(self, x1_t1c,x2_t1n,x3_t2f,x4_t2w,x5_dce,x6_lca,x7_lcv,mask_code):
        # print(x_in.shape, task_id.shape)
        hidden_states_out = self.swinViT(x1_t1c,x2_t1n,x3_t2f,x4_t2w,x5_dce,x6_lca,x7_lcv,mask_code, self.normalize)
        # print(hidden_states_out[0].shape,hidden_states_out[1].shape,hidden_states_out[2].shape,hidden_states_out[3].shape,hidden_states_out[4].shape)
        # enc0 = self.encoder1(x_in)
        # enc1 = self.encoder2(hidden_states_out[0])
        # enc2 = self.encoder3(hidden_states_out[1])
        # enc3 = self.encoder4(hidden_states_out[2])
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.patch_embed_t1c = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        #     spatial_dims=spatial_dims,
        # )
        # self.patch_embed_t1n = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        #     spatial_dims=spatial_dims,
        # )
        # self.patch_embed_t2f = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        #     spatial_dims=spatial_dims,
        # )
        # self.patch_embed_t2w = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        #     spatial_dims=spatial_dims,
        # )
        # self.patch_embed_dce = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        #     spatial_dims=spatial_dims,
        # )
        # self.patch_embed_lca = PatchEmbed(
        #     patch_size=self.patch_size,
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
        ...
        self.patch_embed_lcv = PatchEmbed(
            patch_size=self.patch_size,
            in_chans=in_chans,
            embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None,  # type: ignore
    ...
    def forward(self, x, normalize=True):
        ...
        x0 = self.pos_drop(x0)# torch.Size([2, 48, 48, 48, 48])
        x0_out = self.proj_out(x0, normalize)# torch.Size([2, 48, 48, 48, 48])
    ...
    # # 加权合成特征，基于 mask_code
    #     aggrevate_feature = (
    #     x0_t1c * mask_code[:, 0:1, None, None, None] +
    #     x1_t1n * mask_code[:, 1:2, None, None, None] +
    #     x2_t2f * mask_code[:, 2:3, None, None, None] +
    #     x3_t2w * mask_code[:, 3:4, None, None, None] +
    #     x4_dce * mask_code[:, 4:5, None, None, None] +
    #     x5_lca * mask_code[:, 5:6, None, None, None] +
    #     x6_lcv * mask_code[:, 6:7, None, None, None]
    # )
    #     #非0模态的数量
    ...
    #     x0 = self.pos_drop(avg_feature)# torch.Size([2, 48, 48, 48, 48])
    #     x0_out = self.proj_out(x0, normalize)# torch.Size([2, 48, 48, 48, 48])
    #     x1 = self.layers1[0](x0.contiguous())
    #     x1_out = self.proj_out(x1, normalize)
    #     x2 = self.layers2[0](x1.contiguous())
